...
from scratch_ml.supervised_learning import RidgeRegression
...
def main():
    ...
    model = RidgeRegression(
        reg_factor=0.05,   learning_rate=0.001, n_iterations=1000)
    ...
    plt.plot(400*x, y_pred_line, color='black',
             linewidth=2, label="Prediction")
    plt.suptitle("Ridge Regression")
    plt.title("MSE: %.2f" % mse, fontsize=10)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend((train, test), ("Training data",
